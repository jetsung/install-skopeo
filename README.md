...
Installs Skopeo in GitHub Actions runner. Source: [https://github.com/podman-container-tools/skopeo](https://github.com/podman-container-tools/skopeo)
...
- `version` (optional): Skopeo version.
  ...
  - For source build: Leave empty for the latest stable release from `podman-container-tools/skopeo`. Use `dev` or `master` to build from the respective branches of `podman-container-tools/skopeo`.
...
with:
  mode: 'build'
  version: "1.21.0" # Build a specific version (e.g., v1.21.0 tag from podman-container-tools/skopeo)
